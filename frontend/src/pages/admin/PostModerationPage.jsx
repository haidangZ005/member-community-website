...
export default function PostModerationPage() {
  ...
  return (
    <>
      ...
      <form className="admin-toolbar" onSubmit={submitSearch}><div className="admin-search"><Search size={17} /><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Tìm tiêu đề, nội dung hoặc tác giả…" /></div><select value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }}><option value="">Mọi trạng thái</option><option value="published">Đang hiển thị</option><option value="removed">Đã gỡ</option></select><button type="submit">Tìm kiếm</button></form>
